...
def telegram_input(asm):
    ...
    text and the single most-recently generated .ogg voice reply.
    """
    load_dotenv()
    ...
    async def _handle_update(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async def runner():
            try:
                # 1) Switch TTS to file mode
                asm.tts.set_mode("file")

                # 2) Run your assembler pipeline in a thread
                final = await asyncio.to_thread(asm.run_with_meta_context, user_text)

                # 3) Edit the message with the final text
                ...
                # 4) Send only the single most-recent .ogg file, if any
                try:
